...
### 8.1. Обязательные проверки при тестировании пользователей и ролей
- Сценарии регистрации и управления пользователями должны учитывать обязательное подтверждение чекбокса согласия с Политикой конфиденциальности и Правилами сайта.
- Негативный сценарий обязателен: регистрация без подтверждения чекбокса должна отклоняться backend (`terms_accepted = false` или отсутствие поля).
- Позитивный сценарий обязателен: регистрация с подтвержденным чекбоксом должна успешно создавать пользователя.
- При тестировании админ-функций пользователей (смена роли, блокировка, отзыв сессий, удаление) фиксировать в audit факт действия и его результат.
