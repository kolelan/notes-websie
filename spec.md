...
### 7.1. UI и производительность (текущий этап)
- UI-библиотека: **Ant Design** для таблиц, форм, меню, модалок и пагинации.
- Иконки: **Font Awesome**.
- До размера клиентского bundle **1 MB** (minified JS) оптимизации по code-splitting не являются блокером.
- При превышении 1 MB: включить lazy-loading роутов и дополнительное разбиение чанков.
...
### 11.7. Стратегия первого superadmin
- Используется CLI-команда первого запуска: `create:superadmin`.
- Команда идемпотентна:
  - если пользователя с `SUPERADMIN_EMAIL` нет — создаёт;
  - если есть — обновляет имя/пароль и фиксирует роль `superadmin`.
- Данные берутся из переменных окружения:
  - `SUPERADMIN_EMAIL`,
  - `SUPERADMIN_NAME`,
  - `SUPERADMIN_PASSWORD`.
- Запуск команды выполняется после миграций в деплой-скрипте.
- Пароль хранится только в виде хэша (`password_hash`), в git не коммитится.
- После первого успешного запуска рекомендуется сменить пароль и удалить/очистить `SUPERADMIN_PASSWORD` из постоянной конфигурации.