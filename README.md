...
### Auth
- `POST /auth/register` (email/name/password + `terms_accepted=true`, выдает access/refresh)
...
- регистрация требует явного согласия с Политикой/Правилами (`terms_accepted`);
- `notes` маршруты защищены bearer access token;
- добавлен `groups CRUD` с иерархией через `parent_id` и защитой от циклов.

## Тестовые пометки по пользователям и ролям

- При тестировании регистрации обязательно проверять два кейса:
  - без подтверждения чекбокса Политики/Правил регистрация должна быть отклонена;
  - с подтверждением чекбокса регистрация должна проходить успешно.
- При тестировании админ-функций пользователей (смена ролей, блокировка, отзыв токенов, удаление) учитывать аудит действий и результат операции.
